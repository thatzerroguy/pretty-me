...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => (
  <div className="group animate-fade-in-up">
    <div className="relative aspect-[3/4.5] overflow-hidden mb-5 bg-gray-50 border border-gray-100">
      ...
      />
      {product.isNew && (
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 text-[9px] uppercase tracking-widest font-bold shadow-sm">
          New
        </div>
      )}
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white/95 text-center py-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out shadow-inner backdrop-blur-sm">
        <button className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-900">
          Quick Add
        </button>
      </div>
    </div>
    ...
    <div className="text-center">
      <h3 className="font-medium text-gray-900 text-[13px] md:text-sm mb-1 uppercase tracking-wider">
        {product.name}
      </h3>
      <p className="text-gray-400 text-[11px] mb-3 italic">
        {product.category}
      </p>
      ...
      <p className="text-sm font-semibold tracking-wide">
        {product.discountPrice ? (
          <>
            ...
            <span className="text-gray-400 line-through font-normal ml-2 text-xs">
  ...
  return (
    <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex justify-between items-end mb-16">
        ...
        <Link
          href="/dresses"
          className="group flex items-center text-[11px] uppercase tracking-[0.2em] font-bold text-gray-500 hover:text-black transition-colors"
        >
          View All{" "}
      ...
      <div className="mt-16 text-center sm:hidden">
        <Link
          href="/dresses"
          className="inline-block border border-gray-200 px-10 py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:border-black transition-colors"
        >
          Explore Arrivals
        </Link>
      </div>
